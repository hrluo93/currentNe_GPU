 ...
 * - Optional GPU path (USE_CUDA_NE) with automatic fallback to CPU
 * - Use long long for x_* pair counters to match CUDA atomics
 * - Keep original math and I/O behavior
 */
...
char indi[MAXIND][MAXLOCI]={'\0'}; // Genotipos: 0,1,2, 9=missing
char base[MAXLOCI]={'\0'};
double mapposi[MAXLOCI]={0};
...
double chrprop[MAXCROMO]={0}, chrsize[MAXCROMO]={0};
double chrmin[MAXCROMO]={0},chrnum[MAXCROMO]={0};
...
// --- pair accumulators: use long long for counts ---
long long x_containdX[MAXLOCI]={0};
long long x_contapares[MAXLOCI]={0};
double xD[MAXLOCI]={0},xW[MAXLOCI]={0};
long long x_containdX05[MAXLOCI]={0};
long long x_contapares05[MAXLOCI]={0};
long long x_containdXlink[MAXLOCI]={0};
long long x_contapareslink[MAXLOCI]={0};
double xD05[MAXLOCI]={0},xW05[MAXLOCI]={0};
double xDlink[MAXLOCI]={0}, xWlink[MAXLOCI]={0};
...
double posiCM[MAXLOCI] = {0};
double posiBP[MAXLOCI] = {0};
...
struct AppParams {
    ...
    AppParams()
        : numThreads(0), numSample(0), numSNPs(0), Mchr(1.0), umbral(40),
          K(0), ks(0), flagks(false), flagnok(true), miss(0.2),
          quiet(false), printToStdOut(false), verbose(false), z(0.0), progress() {}
};

struct AppParams params;  // default ctor
...
void readFile_ped(std::string fichinput1, std::string fichinput2, char (&population)[MAXIND][MAXLOCI], PopulationInfo(&popInfo))
{
    ...
    entrada.open(fichinput1, std::ios::in);
    if (!entrada.good())
    {
    ...
    while (std::getline(entrada, line))
    {
        ...
        if (conta == 6)
        {
            ...
            while (posi < longi)
            {
                ...
                rightletter = true;
                str_base = base1[0];
                if (str_plink.find(str_base) == str_plink.npos) rightletter = false;
                str_base = base2[0];
                if (str_plink.find(str_base) == str_plink.npos) rightletter = false;
                if (!rightletter)
                {
                    std::cerr << "Wrong allele in line " << nline << " of ped file." << std::endl;
                    exit(EXIT_FAILURE);
                }
                if ((base1[0] != '0') && (base2[0] != '0') && (base1[0] != 'N') && (base2[0] != 'N') && (base1[0] != 'n') && (base2[0] != 'n'))
                {
                    if (base[popInfo.numLoci] == '\0') base[popInfo.numLoci] = base1[0];
                    if (base1[0] != base[popInfo.numLoci]) base1[0] = 'X';
                    if (base2[0] != base[popInfo.numLoci]) base2[0] = 'X';

                    if (base1[0] == base2[0])
                    {
                        if (base1[0] == base[popInfo.numLoci]) population[popInfo.numIndividuals][popInfo.numLoci] = 0;
                        else population[popInfo.numIndividuals][popInfo.numLoci] = 2;
                    }
                    else population[popInfo.numIndividuals][popInfo.numLoci] = 1;
                }
                else population[popInfo.numIndividuals][popInfo.numLoci] = 9;
            ...
            if (popInfo.numIndividuals == 0) contaLociBase = popInfo.numLoci;
    ...
    flag_chr = false;
    ncromos = 0;
    entrada.open(fichinput2, std::ios::in);
    if (entrada.good()) flag_chr = true;
    if (flag_chr)
    {
        int contalines = 0;
        std::string line;
        while (std::getline(entrada, line))
        {
            ...
            std::string cromocod = line.substr(0, posi);
            static std::string cromocodback = "laksjhbqne";
            ...
            ++posi;  // SNP name (skip)
            posi=static_cast<int>(line.find_first_of(" \t",posi));
            if (posi <= 0) { std::cerr << "Error in map file (1)" << std::endl; exit(EXIT_FAILURE); }
            ++posi;  // cM
            int posi2 = posi;
            posi=static_cast<int>(line.find_first_of(" \t",posi2));
            if (posi <= 0) { std::cerr << "Error in map file (2)" << std::endl; exit(EXIT_FAILURE); }
            posiCM[contalines] = std::stod(line.substr(posi2, posi - posi2));
            if (posiCM[contalines]>0){flag_cM=true;}
            ++posi;
            if (posi > longi) {  std::cerr << "Error in map file (3)" << std::endl; exit(EXIT_FAILURE); }
        ...
        if (flag_chr)
        {
            ...
            {
                chrnum[i] = rangocromo[i + 1] - rangocromo[i];
                chrprop[i] = chrnum[i] / contalines;
                if (flag_Gs){
                    chrsize[i] = chrprop[i] * genomesize;
                }
                else if (flag_cM){
                    chrsize[i] = (posiCM[rangocromo[i+1]-1]-posiCM[rangocromo[i]])/100;
                }
...
void readFile_tped(std::string fichinput1, char (&population)[MAXIND][MAXLOCI], PopulationInfo(&popInfo))
{
    ...
    while (std::getline(entrada, line))
    {
        ...
        if (conta == 4)
        {
            ...
            while (posi < longi)
            {
                ...
                if (!rightletter)
                {
                    std::cerr << "Wrong allele in line " << nline << " of tped file." << std::endl;
                    exit(EXIT_FAILURE);
                }
                if ((base1[0] != '0') && (base2[0] != '0') && (base1[0] != 'N') && (base2[0] != 'N') && (base1[0] != 'n') && (base2[0] != 'n'))
                {
            ...
            if (popInfo.numLoci == 0) contaIndBase = popInfo.numIndividuals;
    ...
    if (flag_chr)
    {
        ...
        if(!flag_Gs){genomesize=Mtot;}
        unomenoschrprop = 0;
        int contalines = popInfo.numLoci;
        for (i = 0; i < ncromos; ++i)
        {
            chrnum[i] = rangocromo[i + 1] - rangocromo[i];
            chrprop[i] = chrnum[i] / contalines;
...
void readFile_vcf(std::string fichinput1, char (&population)[MAXIND][MAXLOCI], PopulationInfo(&popInfo))
{
    ...
    while (std::getline(entrada, line))
    {
        ...
        if (line.size() && line.at(0) != '#')
        {
            ...
            conta = 0;
            posi = 0;
            posi = int(line.find_first_of("\t", posi));
            if (posi < 0) { std::cerr << "Line "<<nline<<" too short in vcf file" << std::endl; exit(EXIT_FAILURE); }
            cromocod = line.substr(0, posi);
            ++posi; ++conta;

            posi2 = posi;
            posi = static_cast<int>(line.find_first_of("\t", posi2));
            if (posi < 0) { std::cerr << "Line too short in vcf file" << std::endl; exit(EXIT_FAILURE); }
            posiBP[popInfo.numLoci] = std::stoi(line.substr(posi2, posi-posi2));
            if (posiBP[popInfo.numLoci]>0){flag_Mb=true;}
            ++posi; ++conta;
            ...
            posi = int(line.find_first_of("\t", posi2));
            j = posi - posi2;
            rightletter = true; hayalelos = true;
            for (i = 0; i < j; ++i)
            {
                str_base = line.substr(posi2 + i, 1);
                if (str_base == ".") hayalelos = false;
                else if (str_vcf.find(str_base) == str_vcf.npos) { rightletter = false; break; }
            }
            if (!rightletter)
            {
                std::cerr << "Wrong reference allele in line " << nline << " of vcf file." << std::endl;
                exit(EXIT_FAILURE);
            }
            ++posi; ++conta;
            ...
            if (!rightletter)
            {
                std::cerr << "Wrong alternative allele in line " << nline << " of vcf file." << std::endl;
                exit(EXIT_FAILURE);
            }
            ++posi; ++conta;
            ...
            haygenotipos = false;
            if (posi < longi && line.substr(posi, 2) == "GT") haygenotipos = true;
...
void printHelp(char * appName) 
{
    fprintf(stderr,
        ...
        "         .vcf, .ped, or .tped input formats are supported.\n"
        "OPTIONS:\n"
        "   -h    Help\n"
        "   -s    Number of SNPs to use (default: all)\n"
        "   -k    Siblings parameter (see manual)\n"
        "   -o    Output filename\n"
        "   -t    Number of threads (OpenMP)\n"
        "   -q    Quiet: print only Ne (and CIs with -v)\n"
        "   -p    Print analysis to stdout (no file)\n"
        "   -v    With -q, also print 50%% and 90%% CI bounds\n",
        appName
    );
}
...
int main(int argc, char * argv[]) {
    ...
    while (punto2!=std::string::npos){
        ...
        if (punto2!=std::string::npos) punto=punto2;
        else break;
    }
    ...
    if (!params.quiet) std::cout <<" Processing file" << std::endl;

    for (j=0;j<popInfo.numIndividuals;++j) pind[j]=j;
    pfinind = &pind[0] + popInfo.numIndividuals - 1;
    prefind = &pind[0];

    for (j=0;j<popInfo.numLoci;++j) ploc[j]=j;

    contaloc2=popInfo.numLoci;
    {
        std::mt19937 g2(rd());
        std::shuffle(&ploc[0],&ploc[popInfo.numLoci],g2);
    }
    prefloc = &ploc[0];
    ...
    if (n_SNPs>0) enelocsolicitado=eneloc=n_SNPs;
    else enelocsolicitado=eneloc=popInfo.numLoci;

    if (n_sample > 0) eneind = n_sample;
    else eneind = popInfo.numIndividuals;

    if (eneloc > popInfo.numLoci) eneloc = popInfo.numLoci;
    if (eneind> popInfo.numIndividuals) eneind=popInfo.numIndividuals;

    // remove invalid
    for (i=eneind-1;i>=0;--i){
        if (!validind[i]){
            for (j=i;j<eneind;++j) pind[j]=pind[j+1];
            --eneind; ++contaelim;
        }
    }

    // freq and segregating
    contaseg=0; containdX=0; pj=prefloc; frecmed=0; nfrecmed=0;
    int mincontaindX = int(float(eneind) * 2.0 * (1 - params.miss));
    double nindmedfs = 0;
    for (j=0;j<popInfo.numLoci;++j){
        frec[*pj]=0; segrega[*pj]=false; homo[*pj]=0;
        pk=prefind; containdX=0;
        for (i=0;i<eneind;i++){
            ff=indi[*pk][*pj];
            if (ff<9){
                frec[*pj]+=ff;
    ...
    for (j2=0;j2<eneloc;++j2){
        for(;;){
            if (segrega[*pj]){
                ...
                ++pj;
                if(pj > pfinloc){ superadolimite=true; break; }
            }
        }
    ...
    // map distances
    if (flag_chr){
        maxdistance=0; maxdistanceindx=0; sumadist=0;
        if (flag_Gs){
            for (i = 0; i < ncromos; ++i){
                if (maxdistance<chrsize[i]) maxdistance=chrsize[i];
                acusize = chrsize[i];
                j=int(acusize/increMorgans);
                if (maxdistanceindx<j) maxdistanceindx=j;
                k=1;
                while ((j>=0)){
                    mapdist[j]+=k;
                    sumadist+=k;
            ...
            for (i = 0; i < ncromos; ++i){
                for (j=rangocromo[i];j<rangocromo[i+1]-1;++j){
                    for (k = j+1; k < rangocromo[i+1]; ++k){
                        d=int(abs(posiCM[k]-posiCM[j])/increMorgans);
        ...
        for (j = 0; j <maxdistanceindx; ++j) mapdist[j]/=sumadist;
    } else {
        sumadist=0;
        maxdistance = 1;
        ...
        while ((j<maxdistanceindx)){
            mapdist[j]+=k; sumadist+=k; ++k; ++j;
        }
        for (j = 0; j <maxdistanceindx; ++j) mapdist[j]/=sumadist;
    }
    if (maxdistance>10){
        std::cerr << "Chromosome sizes are limited to a maximum of 1000 cM." << std::endl;
        return -1;
    }

    fp = (1.0 + fs * (2 * nindmedfs - 1.0)) / (2 * nindmedfs - 1.0 + fs);

    // kinship detection (kept)
    a=4/(6-frecmed*(1-frecmed)) - 0.028;
    nparhermanos=0; npadrehijo=0;
    for (i=0;i<popInfo.numIndividuals-1;++i){
        for (j=i+1;j<popInfo.numIndividuals;++j){
            counthethet=0; counthethomo=0; counthomohomo=0; countnopadre=0;
            for (j2=0;j2<popInfo.numLoci;++j2){
                sumahethomo=indi[i][j2]+indi[j][j2];
                if ((sumahethomo==1) || (sumahethomo==3)){ ++counthethomo; }
                else if (sumahethomo==2){
                    if (indi[i][j2]==indi[j][j2]) ++counthethet;
                    else{ ++counthomohomo; ++countnopadre; }
                }
            }
            if (counthethet>10){
                ...
                ratiohets=0;
                if (b>0) ratiohets=(float(counthethet)+float(counthethomo)/2.0)/b;
    ...
    for (i=0;i<eneind;++i){
        Het[*pk]=0;
        pj=prefloc; contalocX=0;
        for (j2=0;j2<eneloc;++j2){
            for(;;){
                if (segrega[*pj]){
                    ff=indi[*pk][*pj];
                    if (ff==1){ ++Het[*pk]; ++contalocX; }
                    else if (ff<9){ ++contalocX; }
                    ++pj; break;
                } else ++pj;
            }
        }
    ...
    acuHet=0; pk=prefind;
    for (i=0;i<eneind;++i){ acuHet+=Het[*pk]; ++pk; }
    acuHet /= eneind;
    Het_med = acuHet;
    sx2=sx3=0; pk=prefind;
    for (i=0;i<eneind;++i){
        a=(Het[*pk]/acuHet-1);
        sx2+=a*a; sx3+=a*a*a; ++pk;
    }
    ...
    // ===================== Calculo de D2 (GPU if available) =====================
    ...
    acunlink=effndatalink=acuD2link=acuWlink=0;

    // valid indices (heap, not stack)
    std::vector<int> valid_idx;
    valid_idx.reserve(contaloc2);
    for (int idx=0; idx<contaloc2; ++idx) if (segrega[ploc[idx]]) valid_idx.push_back(ploc[idx]);
    if (eneloc > (int)valid_idx.size()) eneloc = (int)valid_idx.size();

    // zero accumulators (length eneloc)
    std::fill_n(x_contapares,    eneloc, 0LL);
    std::fill_n(x_containdX,     eneloc, 0LL);
    std::fill_n(xD,              eneloc, 0.0);
    std::fill_n(xW,              eneloc, 0.0);
    if (flag_chr){
        std::fill_n(x_contapares05, eneloc, 0LL);
        std::fill_n(x_containdX05,  eneloc, 0LL);
        std::fill_n(xD05,           eneloc, 0.0);
        std::fill_n(xW05,           eneloc, 0.0);
        std::fill_n(x_contapareslink, eneloc, 0LL);
        std::fill_n(x_containdXlink,  eneloc, 0LL);
        std::fill_n(xDlink,           eneloc, 0.0);
        std::fill_n(xWlink,           eneloc, 0.0);
    }
    ...
    // try GPU
    int dev_count = 0;
    cudaError_t cuerr = cudaGetDeviceCount(&dev_count);
    if (cuerr == cudaSuccess && dev_count > 0) {
        // column-major genoT: [eneloc columns] × [eneind rows]
        std::vector<char> genoT((size_t)eneind * (size_t)eneloc);
        for (int col=0; col<eneloc; ++col) {
            int loc = valid_idx[col];
            for (int r=0; r<eneind; ++r) {
                int ind = pind[r];
                genoT[(size_t)col * eneind + r] = indi[ind][loc];
            }
        }
        std::vector<int>    cromo_sub(flag_chr ? eneloc : 0);
        std::vector<double> posi_sub(flag_cM ? eneloc : 0);
        if (flag_chr) for (int j=0;j<eneloc;++j) cromo_sub[j] = cromo[ valid_idx[j] ];
        if (flag_cM)  for (int j=0;j<eneloc;++j) posi_sub[j]  = posiCM[ valid_idx[j] ];

        ComputeLD_GPU(
            genoT.data(), eneind, eneloc,
            flag_chr ? cromo_sub.data() : nullptr,
            flag_cM  ? posi_sub.data()  : nullptr,
            flag_chr, flag_cM, params.z,
            x_contapares, x_containdX, xD, xW,
            x_contapares05, x_containdX05, xD05, xW05,
            x_contapareslink, x_containdXlink, xDlink, xWlink
        );
        used_gpu = true;
    }
#endif
    ...
    if (!used_gpu) {
        // CPU OpenMP fallback
        for (int j2=0; j2<eneloc-1; ++j2){
            int* ppj_local = &valid_idx[j2];
            #pragma omp parallel for private(tacui, tacuj,tacuHoHo,distancia,id,tacuHoHetHetHo,tacuHetHet,ppk,ppi,D,W,r2,ss,_containdX) schedule(static)
            for (int j3=j2+1; j3<eneloc; ++j3){
                int* ppi_local = &valid_idx[j3];
                tacuHoHo=tacuHoHetHetHo=tacuHetHet=0;
                tacui = tacuj = 0;
                ppk=prefind;
                _containdX=0;
                for (int _i=0; _i<eneind; ++_i){
                    ss=indi[*ppk][*ppj_local]+indi[*ppk][*ppi_local];
                    if (ss<9){
                        tacui += indi[*ppk][*ppi_local];
                        tacuj += indi[*ppk][*ppj_local];
                        ++_containdX;
                        if (ss==2){
                            if (indi[*ppk][*ppj_local]==indi[*ppk][*ppi_local]){++tacuHetHet;}}
                ...
                if (_containdX>0){
                    ...
                    tacuj /= (_containdX * 2);
                    W = tacui * tacuj;
                    D = -2 * W + (2 * tacuHoHo + tacuHoHetHetHo + tacuHetHet / 2.0) / _containdX;
                    D *= D;
                    W *= (1 - tacui) * (1 - tacuj);
                    ...
                    if (flag_chr){
                        if ((cromo[*ppi_local] != cromo[*ppj_local])){
                            #pragma omp atomic
                            x_contapares05[j3] += 1;
                            #pragma omp atomic
                            ...
                            bool takeLink = true;
                            if (flag_cM){
                                distancia=fabs(posiCM[*ppi_local] - posiCM[*ppj_local]);
                                takeLink = (distancia>params.z);
                            }
                            if (takeLink){
                                #pragma omp atomic
                                x_contapareslink[j3] += 1;
                                #pragma omp atomic
    ...
    if (flag_chr)
    {
        for (int j3 = 0; j3 < eneloc; ++j3)
        {
            acun05 += (double)x_contapares05[j3];
            effndata05 += (double)x_containdX05[j3];
            acuD205 += xD05[j3];
            acuW05 += xW05[j3];
        }
        for (int j3 = 0; j3 < eneloc; ++j3)
        {
            acunlink += (double)x_contapareslink[j3];
            effndatalink += (double)x_containdXlink[j3];
            acuD2link += xDlink[j3];
            acuWlink += xWlink[j3];
        }
    ...
    else
    {
        for (int j3 = 0; j3 < eneloc; ++j3)
        {
            acun     += (double)x_contapares[j3];
            effndata += (double)x_containdX[j3];
            acuD2    += xD[j3];
            acuW     += xW[j3];
        }
        d2s  = acuD2 / acuW;
    ...
    if (params.flagnok){
        if (flagnoestimaks) params.ks=0;
        else params.ks=float(nparhermanos)*2.0/float(popInfo.numIndividuals);
    }

    std::stringstream salida;
    salida << "# (currentNe v1.0)\n";
    salida << "# Command:"; for (i=0;i<argc;++i){salida << " " << argv[i];} salida << "\n";
    salida << "# Running time:" << (float(tpas))<<"sec\n#\n";
    salida << "# INPUT PARAMETERS:\n";
    salida << "# Number of chromosomes in .map file:\n";
    if (flag_chr){
        salida << std::fixed << std::setprecision(0) << ncromos<<"\n";
    } else {
        salida << "There is not chromosome information:\n";
    }
    salida << "# Genome size in Morgans:\n";
    salida << std::fixed << std::setprecision(2) << genomesize<<"\n";
    salida << "# Total number of individuals in the input file:\n";
    salida << std::fixed << std::setprecision(0) << popInfo.numIndividuals << "\n";
    ...
    if (params.flagks || params.flagnok) salida <<"Not given\n#";
    else { salida << params.K<<"\n#"; }
    salida << "# Number of full siblings that a random individuals has in the sample:\n";
    if (params.flagks || params.flagnok){
        if (params.flagnok && flagnoestimaks) salida<< "Not estimated because F value is too large\n#\n";
        else { salida << params.ks<<"\n#\n"; }
    } else { salida <<"Not given\n#\n"; }
    ...
    salida << d2s<<"\n";
    d2_pob=(d2s-(4*double(effeneind)-4)/((2*double(effeneind)-1)*(2*double(effeneind)-1)))/((1-1/(2*double(effeneind)))*0.25);
    if (flag_chr){
    ...
    if (log10_ncrom_sample>log10(60)){log10_ncrom_sample=log10(60);}
    f_pob=fp;
    ...
    for (fciclo=0;fciclo<2;++fciclo){
        ...
        if (params.flagks || params.flagnok){
            ...
            for (i=0;i<30;++i){
                ...
                if ((std::abs(Ne-Neant)<0.1) || (std::abs(Ne-Neant)<(Ne/10000))) break;
                Neant=Ne;
            }
        } else {
            IntegralCromIGUALES();
    ...
    if (!params.quiet)
    {
        if (params.printToStdOut)
        {
            ...
            std::string fichsal="";
            if (fichspecified=="") fichsal= prefijo +"_currentNe_OUTPUT.txt";
            else fichsal= fichspecified;
...
void IntegralCromIGUALES()
{
    ...
    double diststep=increMorgans;
    if (flag_z){
        distmin=diststep/2 + params.z/100;
        ...
        for (i = 0; i < 10; ++i)
        {
            for (conta = 0; conta < DOSMIL; ++conta)
            {
                ...
                while (distancia < maxdistance)
                {
                    ...
                    sumad2spred += mapdist[j] * (d2p * c12 * samplex + ((params.K / 2 + 2 * c2 * K12) / (4 * Ne)) * samplex + sampley) * fp12;
                    distancia += diststep;
                    sumafrec += mapdist[j];
                    ++j;
                }
                ...
                if (abs(increNe) < 0.1) break;
                if (Ne > 100000000){ flagbreak = true; break; }
            }
            if (abs(increNe) < 0.1) break;
            if (flagbreak) break;
        }
...
static inline double linear(double xx){ return xx; }
static inline double sigmoid(double xx){ return 1.0 / (1.0 + exp(-xx)); }
