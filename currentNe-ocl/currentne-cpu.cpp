...
int main(int argc, char * argv[]) {
    ...
    valid_idx.reserve(eneloc);
    for (int idx = 0; idx < contaloc2; ++idx)
    {
        if (segrega[ploc[idx]])
        {
            valid_idx.push_back(ploc[idx]);
            if (int(valid_idx.size()) == eneloc) break;
        }
    }
    if (int(valid_idx.size()) != eneloc)
    {
        std::cerr << "Error: segregating loci index list is inconsistent" << std::endl;
        return -1;
    }
    ...
    for (j3 = 0; j3 < eneloc; ++j3)
    {
        ...
        xD05[j3] = 0;
        xW05[j3] = 0;
        x_contapareslink[j3] = 0;
        x_containdXlink[j3] = 0;
        xDlink[j3] = 0;
        xWlink[j3] = 0;
    }

    bool used_opencl = false;
    std::string opencl_error;

#ifdef USE_OPENCL_NE
    const char* disable_opencl = std::getenv("CURRENTNE_OPENCL_DISABLE");
    if ((disable_opencl == nullptr) || (std::atoi(disable_opencl) == 0))
    {
        try
        {
            ...
            for (j2 = 0; j2 < eneloc; ++j2)
            {
                const int locus_j = valid_idx[j2];
                for (i = 0; i < eneind; ++i)
                {
                    genoT[size_t(j2) * size_t(eneind) + size_t(i)] = indi[prefind[i]][locus_j];
                }
            }

            std::vector<char> cromo_sub;
            std::vector<double> posi_sub;
            if (flag_chr)
            {
                cromo_sub.resize(eneloc);
                for (j2 = 0; j2 < eneloc; ++j2)
                {
                    cromo_sub[j2] = cromo[valid_idx[j2]];
                }
            }
            if (flag_cM)
            {
                posi_sub.resize(eneloc);
                for (j2 = 0; j2 < eneloc; ++j2)
                {
                    posi_sub[j2] = posiCM[valid_idx[j2]];
                }
            }
            ...
            used_opencl = true;
            params.progress.SetTaskProgress(eneloc - 1);
        }
        catch (const std::exception& ex)
        {
            opencl_error = ex.what();
        }
    }
#endif
    ...
    {
#ifdef USE_OPENCL_NE
        if ((!opencl_error.empty()) && (!params.quiet))
        {
            std::cerr << "OpenCL path failed; switching to CPU: " << opencl_error << std::endl;
        }
#endif
...
#pragma omp parallel for
            ...
            {
                const int locus_i = valid_idx[j3];
                double local_tacui = 0;
                double local_tacuj = 0;
                double local_tacuHoHo = 0;
                double local_tacuHoHetHetHo = 0;
                double local_tacuHetHet = 0;
                int local_containdX = 0;
                for (int _i = 0; _i < eneind; ++_i)
                {
                    const int ind_idx = prefind[_i];
                    const int gj = indi[ind_idx][locus_j];
                    const int gi = indi[ind_idx][locus_i];
                    const int local_ss = gj + gi;
                    if (local_ss < 9)
                    {
                        local_tacui += gi;
                        local_tacuj += gj;
                        ...
                        if (local_ss == 2)
                        {
                            if (gj == gi) ++local_tacuHetHet;
                        }
                        else if (local_ss == 3)
                        {
                            ++local_tacuHoHetHetHo;
                        }
                        else if (local_ss == 4)
                        {
                            ++local_tacuHoHo;
                ...
                if (local_containdX > 0)
                {
                    local_tacui /= (local_containdX * 2);
                    local_tacuj /= (local_containdX * 2);
                    double local_W = local_tacui * local_tacuj;
                    double local_D = -2 * local_W + (2 * local_tacuHoHo + local_tacuHoHetHetHo + local_tacuHetHet / 2) / local_containdX;
                    local_D *= local_D;
                    local_W *= (1 - local_tacui) * (1 - local_tacuj);
                    if (flag_chr)
                    {
                        if ((cromo[locus_i] != cromo[locus_j]))
                        {
                            ++x_contapares05[j3];
                            x_containdX05[j3] += local_containdX;
                            xD05[j3] += local_D;
                            xW05[j3] += local_W;
                        }
                        else
                        {
                            if (flag_cM)
                            {
                                distancia = fabs(posiCM[locus_i] - posiCM[locus_j]);
                                if (distancia > params.z)
                                {
                                    ++x_contapareslink[j3];
                                    x_containdXlink[j3] += local_containdX;
                                    xDlink[j3] += local_D;
                                    xWlink[j3] += local_W;
                                }
                            }
                    ...
                    ++x_contapares[j3];
                    x_containdX[j3] += local_containdX;
                    xD[j3] += local_D;
                    xW[j3] += local_W;
                }
            }
